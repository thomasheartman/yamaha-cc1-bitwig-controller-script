...
host.defineController("Yamaha", "CC1", "0.1", "A86F807F-E5BA-4047-916C-391EC950D3C8");
...
function init() {
   ...
   aiKnobParam.parameter().name().addValueObserver(function(name) {
   ...
   faderParam.parameter().name().addValueObserver(function(name) {
      faderParamName = name;
   });
   ...
   faderParam.parameter().value().addValueObserver(function(val) {
      paramValue = val;
      if (faderMode === "param" && !isFaderTouched) pendingFaderValue = val;
   });

   // LED observers
   transport.isPlaying().addValueObserver(            function(v) { setLed("play",   v); });
   transport.isArrangerRecordEnabled().addValueObserver(function(v) { setLed("record", v); });
   transport.isArrangerLoopEnabled().addValueObserver(  function(v) { setLed("loop",   v); });

   cursorTrack.mute().addValueObserver(function(v) { setLed("mute", v); });
   cursorTrack.solo().addValueObserver(function(v) { setLed("solo", v); });
   cursorTrack.arm().addValueObserver( function(v) { setLed("arm",  v); });

   aiKnobParam.isLocked().addValueObserver(function(v) { setLed("lock", v); });

   // HUI button table (zone, port)
   defineButton(0x00, 0, onFaderTouchPress, onFaderTouchRelease);
   defineButton(0x00, 2, function() { cursorTrack.mute().toggle(); }, null, "mute");
   defineButton(0x00, 3, function() { cursorTrack.solo().toggle(); }, null, "solo");
   defineButton(0x00, 7, function() { cursorTrack.arm().toggle();  }, null, "arm");
   defineButton(0x0A, 0, function() { cursorTrack.selectPrevious(); });
   defineButton(0x0A, 2, function() { cursorTrack.selectNext(); });
   defineButton(0x0B, 2, function() { trackPan.reset(); });
   defineButton(0x0D, 5, toggleAIMode, null, "aiMode");
   defineButton(0x0D, 6, lockButton,   null, "lock");
   defineButton(0x0E, 3, function() { transport.stop(); });
   defineButton(0x0E, 4, function() { transport.play(); },   null, "play");
   defineButton(0x0E, 5, function() { transport.record(); }, null, "record");
   defineButton(0x0F, 3, function() { transport.isArrangerLoopEnabled().toggle(); }, null, "loop");
...
function onCC(cc, value) {
   switch (cc) {
      case CC_AI_KNOB:  handleAIKnob(value); break;
      case CC_PAN_KNOB: handleEncoder(trackPan, value); break;
      case CC_FADER_MSB: faderMsb = value; break;
      case CC_HUI_ZONE:  currentHuiZone = value; break;
      case CC_HUI_PORT:  handleHuiPort(value); break;
   }
}
...
function toggleAIMode() {
   ...
   host.showPopupNotification(
      "AI Knob: " + (aiKnobMode === "param" ? "Hover Parameter" : "Vertical Zoom")
   );
}
